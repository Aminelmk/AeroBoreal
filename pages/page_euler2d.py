...
mesh_options = [f"x.{i}" for i in range(1, 10)] + ["Generated Mesh"]
...
def create_surface_plot(variable, title, colorbar_title, x_2d, y_2d):
    ...
    fig = make_subplots(rows=1, cols=1, specs=[[{"type": "surface"}]], subplot_titles=[title])
    fig.add_trace(
        go.Surface(
            x=x_2d,
            y=y_2d,
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(range=[-0.3, 0.3], visible=False),
            yaxis=dict(range=[-1.1, 0.1], visible=False),
    ...
    html.Div([
        ...
        dbc.Card([
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        html.Label("Mesh File"),
                        dcc.Dropdown(
                            id='mesh_selector',
                            options=[{'label': f, 'value': f} for f in mesh_options],
                            value='x.6',
                            clearable=False,
                            className="mb-3"
                        ),
                    ...
                    dbc.Col([
                        ...
                        dcc.Input(id='num_threads', type='number', value=4, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("Mach Number"),
                        dcc.Input(id='Mach', type='number', value=0.5, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("Angle of Attack (α)"),
                        dcc.Input(id='alpha', type='number', value=5.0, className="mb-3 w-100"),
                    ], width=3),
                ], className="mb-4"),
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dcc.Input(id='CFL_number', type='number', value=3.0, step=0.1, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("Pressure (p_inf) [Pa]"),
                        dcc.Input(id='p_inf', type='number', value=1e5, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("Temperature (T_inf) [K]"),
                        dcc.Input(id='T_inf', type='number', value=300.0, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("k2 (2nd Order Dissipation)"),
                        dcc.Input(id='k2', type='number', value=2, step=0.1, className="mb-3 w-100"),
                    ], width=3),
                ], className="mb-4"),
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dcc.Input(id='k4', type='number', value=2, step=0.01, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                        html.Label("Max Iterations"),
                        dcc.Input(id='it_max', type='number', value=10000, className="mb-3 w-100"),
                    ], width=3),
                    dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Button("Run Simulation", id='run_solver', color="primary", className="mt-4 w-100")
                    ], width=3, className="d-flex align-items-end")
                ])
            ])
        ], className="mb-4")
    ], className="p-4", style={'zIndex': 1}),
...
@dash.callback(
    [Output('solver-status', 'children'),
     ...
     Output('selected-mesh-file', 'data')],
    [Input('run_solver', 'n_clicks')],
    [State('mesh_selector', 'value'),
...
def run_simulation(n_clicks, mesh_file, num_threads, Mach, alpha, CFL_number, p_inf, T_inf,
                   multigrid, residual_smoothing, k2, k4, it_max):
    if not n_clicks:
        return dash.no_update, dash.no_update, dash.no_update


    if mesh_file == "Generated Mesh":
        mesh_file_path = "./temp/mesh.xyz"
        #mesh_file_path = "C:/Users/fogue/Downloads/HTML_3D-mesh2d_oihan/mesh.xyz"
        if not os.path.exists(mesh_file_path):
            status = dbc.Alert("❌ Le fichier mesh généré est introuvable. Veuillez d'abord le générer dans la page Mesh.", color="danger")
...
checkpoint_file = checkpoint_test.txt"""
    ...
    try:
        # Accélération : on pourrait ici utiliser numexpr, threading, ou compiler le solver C++
    ...
    return status, redirect, mesh_file
